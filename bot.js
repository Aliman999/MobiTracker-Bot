...
client.on('message', message => {
  ...
  if(command == 'contracts'){
    ...
    var sql = "SELECT u_creator, careertype, price, duration, target, faction, type, unsecure, created_at FROM contracts WHERE faction = 0 "+limit+" ORDER BY created_at DESC;";
    con.query(sql, function (err, result, fields) {
      if(err) throw err;
      console.log(sql);
      if(result.length>0){
